...
import {
    ...
    TableRow, TableCell, TableContainer, FormControl, InputLabel,
...
export default function TaskManagementPage() {
    ...
    React.useEffect(() => { dispatch(fetchTasks()); }, [dispatch]);
    ...
    const onSubmit = async (data: FormData) => {
        ...
        await dispatch(createTask({
            ...
            assignedTo: null, dueDate: data.dueDate,
        }));
        setCreating(false);
        setCreateOpen(false);
        reset();
    ...
    return (
        <Box>
            <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', mb: 3, flexWrap: 'wrap', gap: 2 }}>
                <Box>
                    <Typography variant="h4" fontWeight={800}>Task Management</Typography>
                    <Typography variant="body2" color="text.secondary">Create, manage, and track all tasks.</Typography>
                </Box>
            ...
            <Card>
                {loading ? <Loader message="Loading tasks..." /> : filtered.length === 0 ? (
                    <EmptyState title="No tasks found" description="No tasks match the current filter." />
                ) : (
                    <TableContainer>
                        <Table>
                            <TableHead>
                            ...
                            <TableBody>
                                {filtered.map((task) => (
                                    <TableRow key={task.id} hover>
                                        ...
                                        <TableCell>
                                            <Typography variant="body2" color="text.secondary">{task.createdAt}</Typography>
                                        </TableCell>
                                        <TableCell>
            ...
            <Dialog open={createOpen} onClose={() => { setCreateOpen(false); reset(); }} maxWidth="sm" fullWidth>
                ...
                <DialogContent>
                    <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2.5, pt: 1 }}>
                        <TextField label="Task Title" fullWidth {...register('title')} error={Boolean(errors.title)} helperText={errors.title?.message} />
                        <TextField label="Description" fullWidth multiline rows={3} {...register('description')} error={Boolean(errors.description)} helperText={errors.description?.message} />
                        <Controller
                            name="priority"
                            control={control}
                            render={({ field }) => (
                        ...
                        />
                        <TextField label="Due Date" fullWidth type="date" InputLabelProps={{ shrink: true }} {...register('dueDate')} error={Boolean(errors.dueDate)} helperText={errors.dueDate?.message} />
                    </Box>
                </DialogContent>
                <DialogActions sx={{ px: 3, pb: 2 }}>
                    <Button onClick={() => { setCreateOpen(false); reset(); }} variant="outlined" color="inherit">Cancel</Button>
                    <CustomButton variant="contained" loading={creating} onClick={handleSubmit(onSubmit)}>
                        Create Task
                    </CustomButton>
                </DialogActions>
            </Dialog>
