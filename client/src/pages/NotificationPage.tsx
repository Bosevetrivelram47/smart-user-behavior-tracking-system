...
import {
    ...
    IconButton, Button, Chip, Divider, Card, CardContent, Tooltip,
} from '@mui/material';
import {
...
export default function NotificationPage() {
    ...
    const { data: notifications, loading, error } = useAppSelector((s) => s.notifications);
    const unreadCount = notifications.filter((n) => !n.read).length;

    React.useEffect(() => { dispatch(fetchNotifications()); }, [dispatch]);
    ...
    return (
        <Box>
            <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', mb: 3, flexWrap: 'wrap', gap: 2 }}>
                <Box>
                    <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5 }}>
                        ...
                        {unreadCount > 0 && (
                            <Chip label={`${unreadCount} new`} size="small" color="error" sx={{ fontWeight: 700 }} />
                        )}
                    </Box>
                    <Typography variant="body2" color="text.secondary">Stay updated with the latest activity.</Typography>
                </Box>
                {unreadCount > 0 && (
                    <Button
                        variant="outlined"
            ...
            <Card>
                {loading ? <Loader message="Loading notifications..." /> : notifications.length === 0 ? (
                    <EmptyState
